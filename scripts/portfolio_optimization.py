...
def minimize_volatility(weights, mean_returns, cov_matrix, risk_free_rate):
  ...
  """
  return portfolio_performance(weights, mean_returns, cov_matrix, risk_free_rate)[1]  # Return volatility
...
def efficient_frontier(mean_returns, cov_matrix, risk_free_rate, num_points=100, allow_short_selling=False):
  ...
  for ret in frontier_returns:
      ...
      # Use a default argument in the lambda function to capture the current value of ret
      constraints_with_return = constraints + [{'type': 'eq', 'fun': lambda w, ret=ret: np.dot(w, mean_returns) * 252 - ret}]
      
      result = minimize(
          minimize_volatility,
          x0=np.array([1 / len(mean_returns)] * len(mean_returns)),
          args=(mean_returns, cov_matrix, risk_free_rate),  # Include risk_free_rate
          method='SLSQP',
          bounds=bounds,
      ...
      if result.success:
          ...
          frontier_volatilities.append(np.nan)  # Append NaN if optimization fails

  return frontier_returns, frontier_volatilities


def plot_efficient_frontier(mean_returns, cov_matrix, risk_free_rate, optimal_weights_sharpe, optimal_weights_volatility, frontier_volatilities, frontier_returns, asset_names=None):
  ...
  plt.scatter(
    ...
    portfolio_performance(optimal_weights_sharpe, mean_returns, cov_matrix, risk_free_rate)[0],
    color='r', label='Max Sharpe Ratio Portfolio'
    )
  plt.scatter(
    portfolio_performance(optimal_weights_volatility, mean_returns, cov_matrix, risk_free_rate)[1],
    portfolio_performance(optimal_weights_volatility, mean_returns, cov_matrix, risk_free_rate)[0],
